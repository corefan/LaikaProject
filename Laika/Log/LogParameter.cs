...
namespace Laika.Log
{
	public class LogParameter
	{
		...
		public LogParameter(string path = null, PartitionType type = PartitionType.NONE, string fileName = "log", int time = 60, int size = 1024000, bool debug = false, bool console = false, bool trace = false, string windowsEventSource = "ApplicationSource")
		{
			if (path == null)
				path = Directory.GetCurrentDirectory();
		...
		/// <summary>
		/// trace, debug 모드, file log는 하지 않음.
		/// </summary>
		public bool OnlyTraceDebugMode
		{
			get
			{
				return onlyTraceDebugMode;
			}
			set
			{
